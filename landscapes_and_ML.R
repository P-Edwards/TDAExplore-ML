...
if(opt$landscapes!=FALSE) { 
  training_results <- TDAExplore::TDAExplore(parameters=opt$parameters,number_of_cores=opt$cores,radius_of_patches=opt$radius,patch_ratio=opt$ratio,svm=opt$svm,lower=opt$lower,upper=opt$upper,proportion=opt$proportion,verbose=TRUE,benchmark=opt$benchmark,pca=opt$pca)  

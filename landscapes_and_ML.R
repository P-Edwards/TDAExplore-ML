option_list <- list(optparse::make_option(c("--parameters"),type="character",help="File name of parameters .csv file to use. Relative to current directory or absolute. Required."),
                    ...
                    optparse::make_option(c("--fullsave"),type="numeric",default=FALSE,help="Debugging. If flag is set to FALSE or 0, then no full save is done."),
...
if(opt$plot!=FALSE) { 
  ...
  apply_patch_svm_rotation <- function(data_matrix,classifier=1) { 
    rval <- list()
    ...
    rval$data <- data_matrix%*%(weight_vector) + bias_term
    rval$types <- summaries_results$svm[[classifier]]$testing_labels
  ...
  if(opt$summaries==FALSE) {      
    for(i in 1:number_of_classifiers) {     
      ...
      individual_results[[i]] <- apply_patch_svm_rotation(as.matrix(training_results$svm[[i]]$testing_data),classifier=i)
      individual_results[[i]]$data <- -individual_results[[i]]$data
      individual_image_results[[i]] <- average_by_image(training_results$svm[[i]]$testing_data,function(data_matrix) {return(apply_patch_svm_rotation(data_matrix,classifier=i))},post_transformation_map=function(data_matrix) {return(apply_image_svm_rotation(data_matrix,classifier=i))})
    }
  } else { 
    ...
    individual_image_results[[1]] <- average_by_image(summaries_results$summaries,function(data_matrix) {return(apply_patch_svm_rotation(data_matrix,classifier=1))},post_transformation_map=function(data_matrix) {return(apply_image_svm_rotation(data_matrix,classifier=1))})
  }
  ...
  save(plotting_data,boxplot_image_data,image_file_names,testing_order_image_names,file=file.path(image_results_directory,paste(plotting_name_stem,".RData")),version=2)
}
